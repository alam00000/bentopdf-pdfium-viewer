...
for patch in security-fixes editcore-pdfium; do
  if ! git -C "$SRC" apply --reverse --check "$PDFIUM/build/patch/$patch.patch" >/dev/null 2>&1; then
    git -C "$SRC" apply "$PDFIUM/build/patch/$patch.patch"
  fi
done
