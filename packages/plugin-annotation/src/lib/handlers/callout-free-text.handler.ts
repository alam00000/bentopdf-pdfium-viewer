...
export const calloutFreeTextHandlerFactory: HandlerFactory<PdfFreeTextAnnoObject> = {
  ...
  create(context) {
    ...
    const quarterTurned = pageRotation === 1 || pageRotation === 3;
    const defaultTextBoxSize = quarterTurned
      ? { width: DEFAULT_TB_HEIGHT, height: DEFAULT_TB_WIDTH }
      : { width: DEFAULT_TB_WIDTH, height: DEFAULT_TB_HEIGHT };
    ...
    const buildPreview = (cursor: {
      ...
      if (phase === 'textbox' && arrowTip && knee) {
        ...
        if (getDragging() && tbStart) {
          ...
          textBox = {
            origin: {
              x: cursor.x - defaultTextBoxSize.width / 2,
              y: cursor.y - defaultTextBoxSize.height / 2,
            },
        ...
        if (phase === 'textbox' && getDragging()) {
          ...
          if (tbStart) {
            ...
            if (w > 5 || h > 5) {
              ...
              commitCallout({
                origin: {
                  x: tbStart.x - defaultTextBoxSize.width / 2,
                  y: tbStart.y - defaultTextBoxSize.height / 2,
                },
                size: defaultTextBoxSize,
              });
            }
